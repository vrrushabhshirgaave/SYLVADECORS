...
def get_db_connection():
    try:
        connection_string = (
            f"postgresql+psycopg2://{st.secrets['database']['PG_USER']}:"
            f"{st.secrets['database']['PG_PASSWORD']}@"
            f"{st.secrets['database']['PG_HOST']}:"
            f"{st.secrets['database']['PG_PORT']}/"
            f"{st.secrets['database']['PG_DATABASE']}"
        )
        ...
        st.error("Unable to connect to the database. Please check the logs or contact support.")
        raise
...
def init_db():
    try:
        ...
        st.error("Failed to initialize database. Please check the logs or contact support.")
        raise
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if not result:
                ...
                conn.commit()
                logger.info("Default owner credentials added successfully")
    except Exception as e:
        logger.error(f"Failed to add default owner: {str(e)}")
        st.error("Failed to set up default owner credentials. Please check the logs.")
        raise
...
def verify_login(username, password):
    try:
        ...
        st.error("Error verifying login. Please try again.")
        return False
...
def get_enquiries():
    try:
        ...
        st.error("Failed to fetch enquiries. Please check the logs.")
        raise
...
def generate_pdf(df):
    try:
        ...
        st.error("Failed to generate PDF file. Please try again.")
        raise
...
try:
    ...
    st.error("Application initialization failed. Please check the logs or contact support.")
